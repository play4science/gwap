   ...
   //   optimize( sb );
    ...
    private void strip( StringBuilder buffer )
    {
      ...
      while ( doMore && buffer.length() > 3 ) {
    	  ...
    	  if ( ( buffer.substring( buffer.length() - 3, buffer.length() ).equals( "h%t" ))||
    			//  ( buffer.substring( buffer.length() - 3, buffer.length() ).equals( "k%t" ))||
    			//  ( buffer.substring( buffer.length() - 4, buffer.length() ).equals( "$aft" ))||
    			  ( buffer.substring( buffer.length() - 3, buffer.length() ).equals( "ion" ))||
    			  ( buffer.substring( buffer.length() - 3, buffer.length() ).equals( "nis" ))||
    			  ( buffer.substring( buffer.length() - 3, buffer.length() ).equals( "mus" ))||
    			  ( buffer.substring( buffer.length() - 3, buffer.length() ).equals( "tus" ))){
    		  doMore = false;
    	  }
    	  
    	  else if ( ( buffer.length() + substCount > 5 ) &&
    	          buffer.substring( buffer.length() - 3, buffer.length() ).equals( "l%n" ) )
          {
            buffer.delete( buffer.length() - 3, buffer.length() );       }
    	  else if ( ( buffer.length() + substCount > 5 ) &&
    	          buffer.substring( buffer.length() - 3, buffer.length() ).equals( "li§" ) )
          {
            buffer.delete( buffer.length() - 3, buffer.length() );       }
    	  else if ( ( buffer.length() + substCount > 5 ) &&
    	          buffer.substring( buffer.length() - 2, buffer.length() ).equals( "i$" ) )
          {
            buffer.delete( buffer.length() - 2, buffer.length() );       }
    	  else if ( ( buffer.length() + substCount > 4 ) &&
    	          buffer.substring( buffer.length() - 2, buffer.length() ).equals( "ig" ) )
          {
            buffer.delete( buffer.length() - 2, buffer.length() );       }
    	  
    	  
    	  else if ( ( buffer.length() + substCount > 6 ) &&
          buffer.substring( buffer.length() - 3, buffer.length() ).equals( "end" ) )
        {
          buffer.delete( buffer.length() - 3, buffer.length() );
        }
        else if ( ( buffer.length() + substCount > 4 ) &&
    	  ...
    	  // für weibliche Berufsbezeichnungen
                ...
                buffer.substring( buffer.length() - 2, buffer.length() ).equals( "in" ) ) {
                  buffer.delete( buffer.length() - 2, buffer.length() );
              }
        else if ( ( buffer.length() + substCount > 8 ) &&
                buffer.substring( buffer.length() - 5, buffer.length() ).equals( "in*en" ) ) {
                  buffer.delete( buffer.length() - 5, buffer.length() );
              }
        else if ( ( buffer.length() + substCount > 5 ) &&
                buffer.substring( buffer.length() - 3, buffer.length() ).equals( "§en" ) ) {
                  buffer.delete( buffer.length() - 3, buffer.length() );
              }
        else if ( buffer.charAt( buffer.length() - 1 ) == 'e' ) {
          buffer.deleteCharAt( buffer.length() - 1 );
        }
        else if ( ( buffer.length() + substCount > 4 ) && buffer.charAt( buffer.length() - 1 ) == 's' ) {
          buffer.deleteCharAt( buffer.length() - 1 );
        }
        else if (  buffer.substring( buffer.length() - 2, buffer.length() ).equals("ln") ||
        		buffer.substring( buffer.length() - 2, buffer.length() ).equals("rn") ) {
          buffer.deleteCharAt( buffer.length() - 1 );
        }
    ...
    private void removeParticleDenotion( StringBuilder buffer )
    {
      if ( buffer.length() > 4 ) {
        for ( int c = 0; c < buffer.length() - 3; c++ ) {    	
          if ( buffer.substring( c, c + 2 ).equals( "ge" ) ) {
            buffer.delete( c, c + 2 );
            return;
          }
        }
